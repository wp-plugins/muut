...
<!-- Use Moot instead of Wordpress for comments | https://muut.com */ -->
<style>
...
<?php
  ...
  if ($key) {
    ...
    $signature = sha1(get_option('muut_secret_key') . ' ' . $message . ' ' . $timestamp);
  }
...
$(function() {
  ...
  <?php if (get_option('muut_generate') == "true") { ?>
    $("body").addClass("has-moot");
  <?php } ?>

  if (typeof moot != "function") return $("#moot").remove();

  <?php if (get_option('muut_comments_under_forums')) { ?>
    moot(function(app) {
      if (app.is_forum) {
  ...
  if ($("#no-moot")[0]) return default_moots.remove();
